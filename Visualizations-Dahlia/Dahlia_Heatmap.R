...
progression_nov20$Samples <- tools::toTitleCase(progression_nov20$Samples) #Sentence Case
progression_nov20 <- progression_nov20 %>% mutate(Samples = ifelse(row_number() == 1, "PFS", Samples)) #First row name as PFS
...
gbm_sample_info_combined <- gbm_sample_info_combined %>% mutate(PFS = as.factor(PFS))
rownames(gbm_sample_info_combined) <- gbm_sample_info_combined$Samples
...
plot_and_save_heatmap_pfs(
  ...
  metabGroups_nov20, 
  "Metabolites by Progression", 
  "62_PFS Metabolites Nov20 data -rows cols eucledian ward -split.pdf"
)


####Plotting Both GBM and PFS Status WORKING----

create_heatmap_gbm_pfs <- function(count_scores, pathway, sample_data) {
  # Ensure sample_data is in the same order as count_scores columns
  sample_data <- sample_data[colnames(count_scores), , drop = FALSE]
  
  # Define the desired order for Experiment
  experiment_order <- c(0, 1) #
  pfs_order <- c(0, 1, 2)
  
  # Convert Sex and Experiment to factors with specified levels
  sample_data$PFS <- factor(sample_data$PFS, levels = pfs_order)
  sample_data$GBM <- factor(sample_data$GBM, levels = experiment_order)
  
  # Create color vectors for each annotation
  slide_colors <- setNames(c("#707070", "#FF3079"), experiment_order)#, "#049193", "#5B2897"
  neuron_colors <- setNames(c("#007DEF", "#F08C00", "#DE56C0"), pfs_order)
  ...
  ha_top <- HeatmapAnnotation(
    PFS = sample_data$PFS,
    GBM = sample_data$GBM,
    # col = list(PFS = neuron_colors, Experiment = slide_colors),
    col = list(GBM = slide_colors),
    show_annotation_name = TRUE,
    annotation_name_side = "right",
    gap = unit(2, "mm"),
...
plot_and_save_heatmap_gbm_pfs <- function(normalizedCountsData, sample_data_subset, pathway, output_file) {
  # Calculate height based on number of genes
  # height <- max(12, nrow(normalizedCountsData) * 0.2)  # Adjust the multiplier (0.2) as needed
  height <- 19
  width <- 11 #16 or 10.5
  ...
  pdf(output_file, width = width, height = height)  # Increased width to accommodate legends
  create_heatmap_gbm_pfs(normalizedCountsData, pathway, sample_data_subset)
  dev.off()
  print(create_heatmap_gbm_pfs(normalizedCountsData, pathway, sample_data_subset))
  print(paste("Heatmap for", pathway, "pathway saved to", output_file))
}
...
# Plot and save the heatmap
plot_and_save_heatmap_gbm_pfs(
  scaled_data, #data_metab_final or scaled_data
  gbm_sample_info_combined, 
  "GBM Metabolites", 
  "27_GBM_Metabolite_Combined PFS GBM.pdf"
)
